...
# Pull all per-respondent CSVs from S3 and combine
s3_client <- paws.storage::s3()
objects    <- s3_client$list_objects_v2(
  Bucket = Sys.getenv("S3_BUCKET"),
  Prefix = "responses/"
)
if (length(objects$Contents) == 0) stop("No responses found in S3 bucket.")

d <- do.call(rbind, lapply(objects$Contents, function(obj) {
  raw <- s3_client$get_object(Bucket = Sys.getenv("S3_BUCKET"), Key = obj$Key)
  read.csv(text = rawToChar(raw$Body), stringsAsFactors = FALSE)
}))

# Reference data
d_geo    <- st_read("data/00mun_simplified.geojson", quiet = TRUE) |> st_drop_geometry()
...
d <- d |> filter(!is.na(Found_Municipality_ID) & Found_Municipality_ID != "")

# Treatment factor and dummies (control = baseline)
d <- d |>
  mutate(
    Treatment_Group = factor(Treatment_Group, levels = c("control", "T1", "T2", "T3", "T4")),
...
d <- d |>
  mutate(
    ...
    dY_nonmorena = Coalition_PAN_PRI_PRD_Crime_Rating_Post - Coalition_PAN_PRI_PRD_Crime_Rating_Pre,
    ...
    rank_pre_avg  = rowMeans(cbind(rank_pre_1,  rank_pre_2,  rank_pre_3,  rank_pre_4),  na.rm = TRUE),
    rank_post_avg = rowMeans(cbind(rank_post_1, rank_post_2, rank_post_3, rank_post_4), na.rm = TRUE),
...
vote_includes_coalition <- function(vote_str, coalition) {
  if (is.na(vote_str) || vote_str == "") return(NA_integer_)
...
d <- d |>
  ...
  mutate(
    incumbent_coalition = if_else(
      coalition_label == "A", list(coalition_a), list(coalition_b)
    ),
    dY_vote_pre  = vote_includes_coalition(Vote_Intention_Pre,  unlist(incumbent_coalition)),
    dY_vote_post = vote_includes_coalition(Vote_Intention_Post, unlist(incumbent_coalition)),
...
get_actual_rank <- function(home_id, c1, c2, c3, c4) {
  ...
  if (all(is.na(counts))) return(NA_real_)
...
d <- d |>
  ...
  mutate(
    R_actual = get_actual_rank(
      ...
      Comparison_Muni_1_ID, Comparison_Muni_2_ID,
...
d <- d |>
  left_join(robo_lookup |> rename(robos_home = robos),
...
outcomes <- c("dY_home", "dY_morena", "dY_nonmorena", "dY_mc", "dY_turnout", "dY_rank", "dY_vote")
outcome_labels <- c(
  "Home crime handling",
  "MORENA coalition",
...
model_crime_importance <- lm_robust(
  dY_home ~ T1 + T2 + T3 + T4 +
    W_i + W_i:T1 + W_i:T2 + W_i:T3 + W_i:T4 +
    C_i_std + C_i_std:T1 + C_i_std:T2 + C_i_std:T3 + C_i_std:T4,
  data    = d_full,
  se_type = "HC2"
)
