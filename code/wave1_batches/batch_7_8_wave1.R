...
excluded_pids <- c(
  ...
  read.csv("data/temp/batch_7_7_wave1_pids.csv")$Netquest_PID
)
...
overlap <- intersect(batch_7_8$Netquest_PID, excluded_pids)
if (length(overlap) > 0) {
  stop(sprintf(
...
write.csv(batch_7_8, "data/temp/batch_7_8_wave1_pids.csv", row.names = FALSE)
